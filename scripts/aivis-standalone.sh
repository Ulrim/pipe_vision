...
# --- 옛 버전 조기 감지 ------------------------------------------------------
# 현장 운영 도구가 없는 옛 코드를 받아두면 사용자는 "No such file or directory"
# 만 보고 원인을 알 수 없다. 여기서 미리 알려준다(막지는 않는다 — 기동 자체는
# 가능하므로). 지금은 기본 브랜치(main)에 모두 반영돼 있어 main 으로 맞추면 된다.
if [ -z "${AIVIS_SKIP_BRANCH_CHECK:-}" ] && command -v git >/dev/null 2>&1; then
  if [ ! -f "$REPO/scripts/aivis.sh" ] || [ ! -f "$REPO/scripts/aivis-update.sh" ]; then
    ...
    echo "[standalone]    git fetch origin main"
    echo "[standalone]    git checkout -B main origin/main"
    echo "[standalone] ================================================================"
  fi
fi
