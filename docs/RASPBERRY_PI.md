...
> 캘리브레이션은 대시보드에서도 가능하다: 기준정보 → 품목 편집 → **캘리브레이션** 섹션에
> 시스템 측정값/실제값을 입력하면 `POST /master/items/{code}/calibrate` 가
> `px_to_mm_scale` 을 자동 보정한다(품질관리자 이상, 변경 이력 기록). 단계별 절차와
> 반복성 확인은 `docs/FIELD_SETUP.md` §5 참조.

### 7.3 다중 튜브 모드 (expected_count)

한 프레임에 나란히 눕힌 튜브 N개를 동시에 검사하는 모드. 워커 코드/환경변수 변경 없이
**기준정보 값 하나로 전환**된다(`item_master.expected_count`).

1. 대시보드 → 기준정보 → 품목 편집 → **개수(오더당 튜브)** 에 N 입력(1=단일, 2~20=다중).
   필요 시 외경(mm)도 함께 입력한다.
2. **워커 재시작**(`sudo systemctl restart aivis-vision`) — 워커는 기동 시 1회
   기준정보를 읽으므로 재시작해야 반영된다.
3. 이후 워커는 자동으로 배치 모드로 동작한다: 1프레임 → 튜브 N개 분리(밝기 골 seam 검출)
   → 튜브별 개별 판정 → 결과 N건 POST(원본/배치 오버레이 이미지는 배치당 1장 공유,
   `tube_index` 로 구분).
4. 화면: 결과 이미지에 튜브별 색 박스·번호(#1..#N)와 `BATCH: 판정 tubes 검출/기대 NG n`
   헤더가 표기되고, HMI 는 같은 배치를 **배치 카드 1장**(튜브별 그리드)으로 묶어 보여준다.
5. **개수 불일치 경고**: 독립 자동 검출 개수가 기대 개수와 다르면 배치 판정이 NG 가 된다.
   실제 투입 수량 확인 → 아니라면 조명/배경(seam 검출) 문제다.

배치 요건: 튜브 밀착은 허용되나 **단일 층·상호 평행·모든 튜브 양 끝단 프레임 내 노출**이
필수다. 조명/배경 조건, seam 파라미터 현장 보정, 증상별(과분할/미분할) 대응은
**`docs/FIELD_SETUP.md` §6~§7** 을 따른다.
...
- 현장 광학 셋업·캘리브레이션·다중 튜브 운영: `docs/FIELD_SETUP.md`
